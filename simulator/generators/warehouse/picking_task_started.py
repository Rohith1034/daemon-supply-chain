...
def generate_picking_task_started(task_id):
    with Database() as db:
        task = db.fetch_one(
            """
            SELECT
        ...
        if not task:
            raise Exception("Picking task not found")

        if task["status"] != "CREATED":
            raise Exception(
                f"""
Picking task must be CREATED
CURRENT:
{task['status']}
"""
            )
        ...
        payload = {
            ...
            "correlationId": str(task["correlation_id"]),
        }
...
if __name__ == "__main__":
    try:
        ...
        generate_picking_task_started(sys.argv[1])
    except Exception as e:
        log_event_failure(EVENT_NAME, e)
        raise
